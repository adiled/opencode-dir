...
export const tui: TuiPlugin = async (api) => {
  ...
  // Vault auto-popup when server signals need
  try {
    const home = (process.env.HOME || "") as string
    const base = (process.env.XDG_DATA_HOME || home + "/.local/share") as string
    setInterval(() => {
      try {
        const cur = (api.route.current as any)?.params?.sessionID as string | undefined
        if (!cur) return
        const need = `${base}/opencode/opencode-dir/vault-need-${cur}`
        if (!fs.existsSync(need)) return
        fs.rmSync(need, { force: true } as any)
    ...
    ;(api as any).keymap?.registerLayer?.({
      commands: [{ title: "Vault: set passphrase", value: "opencode-dir.vault.pass", description: "Set vault passphrase for this session" }],
      bindings: [{ command: "opencode-dir.vault.pass", keys: "ctrl+shift+v" } as any],
      handler: async (cmd: string) => {
        if (cmd !== "opencode-dir.vault.pass") return false
        const cur = (api.route.current as any)?.params?.sessionID as string | undefined
        const sid = cur || (api.state as any).session?.get ? "" : ""
          ...
          api.ui.dialog.replace(() => (
            <api.ui.DialogPrompt title="Vault passphrase" placeholder="enter passphrase" onConfirm={(val: string) => {
              try {
                ...
                const base = (process.env.XDG_DATA_HOME || home + "/.local/share") as string
                const file = `${base}/opencode/opencode-dir/vault-pass-${cur || sid}`
                if (cur || sid) { fs.mkdirSync(path.dirname(file), { recursive: true } as any); (fs as any).writeFileSync(file, val, { mode: 0o600 } as any); (api.ui as any).toast?.({ message: "Vault passphrase set" }) }
              } catch {}
              api.ui.dialog.clear(); resolve()
            }} onCancel={() => { api.ui.dialog.clear(); resolve() }} />
          ))
        })
        return true
      },
